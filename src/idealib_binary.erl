...
-export ([concat/1, to_hex/1, to_hexbin/1, to_hexlist/1]).
...
to_hex (Bin) -> to_hexlist (Bin). %% XXX: Compat (slower)

to_hexbin (Bin) when is_binary (Bin) ->
    to_hex (Bin, <<>>);
to_hexbin (Bin) when is_list (Bin) ->
    to_hexbin (iolist_to_binary (Bin)).

to_hexlist (Bin) when is_binary (Bin) ->
    binary_to_list (to_hex (Bin, <<>>));
to_hexlist (Bin) when is_list (Bin) ->
    to_hexlist (iolist_to_binary (Bin)).

-define (H (X), (hex (X)):16).

to_hex (<<>>, Acc) -> Acc;
to_hex (Bin, Acc) when byte_size (Bin) band 7 =:= 0 ->
    to_hex_ (Bin, Acc);
to_hex (<<X:8, Rest/binary>>, Acc) ->
    to_hex (Rest, <<Acc/binary, ?H (X)>>).

to_hex_ (<<>>, Acc) -> Acc;
to_hex_ (<<A:8, B:8, C:8, D:8, E:8, F:8, G:8, H:8, Rest/binary>>, Acc) ->
    to_hex_ (Rest, <<Acc/binary,
                     ?H (A), ?H (B), ?H (C), ?H (D),
                     ?H (E), ?H (F), ?H (G), ?H (H)>>).
...
to_hex_test () ->
    ...
    RandBin = fun (Len) ->
                          << <<(random:uniform (255))>> || _ <- lists:seq (1, Len) >>
              end,
    ?assertEqual ("ff000ff1", to_hex ([255, 0, 15, 241])),
    [ begin
          R = RandBin (1024), H = ToHexSlow (R),
          ?assertMatch (H, to_hex (R))
      end
      || _ <- lists:seq (1, 128) ],
    ok.
